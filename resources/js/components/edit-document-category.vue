    ...
    <div class="mb-5">
        <div class="p-4 rounded bg-gray-200">
            ...
            <div class="flex justify-between">
                ...
                <button v-on:click="$emit('cancel-update-category')" type="button" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    Cancel
                </button>
            </div>
        </div>
...
<script>
export default {
    emits:['cancel-update-category','updated-category'],
    props:['category'],
    mounted() {
    ...
    methods:{
        save(){
            ...
            window.axios.put('/api/document-categories/',{
                ...
                vm.$emit('updated-category',response.data);
            });
        }
    }
}
